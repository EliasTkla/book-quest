...
function Books() {
  ...
  return (
    <>
    <form id='search-bar'>
      <input type="text" placeholder="Title, author, genre, or keyword" value={searchKey} onChange={e => setSearchKey(e.target.value)} onKeyDown={event => {if (event.key === 'Enter') {searchBook()}}}/>
      <button className="fa-solid fa-magnifying-glass" onClick={()=> {searchBook()}}></button>
    </form>
